def main():
    ...
    ## dp[i][j]:= i項で総和Sをつくるような数列Aiのi項めでjを使えるパターンの数
    ## j>=6について、jは2つの項に分けることができるので、それを分配していく
    MOD = 10**9+7
    max_i = s//3
    dp = [[0]*(s+2) for _ in range(max_i+1)]
    dp[1][s] = 1
    ans = 1

    for i in range(1, max_i):

        for j in range(s-3*(i-1), 5, -1):
            if dp[i][j] > 0:
                dp[i+1][3] += dp[i][j]
                dp[i+1][j-3+1] -= dp[i][j]

        csum = list(accumulate(dp[i+1]))
        dp[i+1] = csum
