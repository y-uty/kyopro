def main():
    ...
    Box = CandyBox()
    # for i in range(10): print(Box.masu[i])
    # print('-----')

    for i in range(1, 101):
        pi = int(input())

        Box.Put_Candy(pi, i, F[i-1])
        d = random.randint(0, 3)
        Box.Tilt(Directions[d])
        print(Directions[d])
        sys.stdout.flush()

    # i, p = 1, 3
    # Box.Put_Candy(p, i, F[i-1])
    # for i in range(10): print(Box.masu[i])
    # print('-----')
...
class CandyBox():
    ...
    def __init__(self):
        # 空きが0~99
        # キャンディありは、10**f+(0~99)
        self.masu = [list(range(10*i, 10*i+10)) for i in range(10)]
        # 空きマス番号 to マスの座標
        self.blank_place = collections.defaultdict(int)
        for i in range(100): self.blank_place[i] = (i//10, i%10)

    # 入力piに従って、箱にキャンディを詰める
    def Put_Candy(self, p, input_count, f):
        p -= 1 # 0-indexed
        input_count -= 1
        put_r, put_c = self.blank_place[p]
