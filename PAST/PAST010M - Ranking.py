def main():
  ...
  S = list(map(str, input().split())) # P[i] = 人iの名前

  avl = AVLTree() # 平衡二分探索木で(名前, 生徒番号)を管理して出し入れする
  for i, s in enumerate(S):
    no = i+1
    avl.insert((s, no)) # key: (名前, 生徒番号), val: 生徒番号(1-indexed)
  ...
  for _ in range(Q):
    ...
    nm_bef, no = avl.find_kth_element(X)
    avl.delete((nm_bef, no))
    avl.insert((T, no))
  
  # 最終結果の出力
  ansdict = avl.get_key_val()
  anslist = ['']*(N+1)
  for k, _ in ansdict.items():
    nm, no = k
    anslist[no] = nm
