def main():
    ...
    # Piに重複がないので、AVL木でいけそう

    ans = [-1]*(n+1) # iが書かれたカードが食べられるのが何ターン目か？
    avl = AVLTree()
    avl.insert(-1) # 空でlower_boundするとerrorなるので入れとく

    for i in range(n):
        turns = i+1
        drawn = p[i]
        tmp = avl.lower_bound(drawn) # 引いたカード以上の数字で最小のものは？

        if tmp is None: # ないときは、insert ただし、k=1のケースは即ans更新
            if k>1:
                avl.insert(drawn, set([drawn]))
            else:
                ans[drawn] = turns
        else: # あるときは、key, valを取り出して、新たなkey=Pi, val=val.add(Pi)とする
            piled = avl.get(tmp)
            piled.add(drawn)
            
            if len(piled)==k: # k枚積み重なったら、食べる -> ansの更新
                for _ in range(len(piled)):
                    xx = piled.pop()
                    ans[xx] = turns
            else: # k枚以下であれば、insertで戻す
                avl.insert(drawn, piled)
            
            avl.delete(tmp) # 元の番号は食べられるか下に行くので削除
