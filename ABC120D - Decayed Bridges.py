def main():
    ...
    # 辺を逆順に追加することでUnion-Findを使う
    uf = UnionFind(n)
    unconnected = comb(n, 2)
    ans = [unconnected]
    ...
    for i in range(m-1):
        ...
        # 同じグループのときは既に往来できるため不便さに変化なし
        if a_root==b_root:
            pass
        
        # 異なるグループのとき、それぞれで往来できた組の数と、
        # 1つのグループに繋がったあとの組の数との差が、不便さの変化
        else:
            # Union前のrootが持っている各グループの島の数(=頂点数)から組の数を調べる
            a_comb = comb(-uf.parents[a_root], 2)
            b_comb = comb(-uf.parents[b_root], 2)
            uf.union(a, b)
            # Union後の組の数を調べる
            ab_root = uf.find(a)
            ab_comb = comb(-uf.parents[ab_root], 2)
            # 不便さの変化を求める
            new_connected = ab_comb - (a_comb+b_comb)
            unconnected -= new_connected

        ans.append(unconnected)
